...
from models import Conv2DReparam
...
class ResBlockRparam(Model):
    def __init__(self, channels, stride=1):
        super(ResBlockRparam, self).__init__(name='ResBlockReparam')
        self.flag = (stride != 1)
        self.conv1 = Conv2DReparam(channels, [3, 3], stride, padding='SAME')
        self.bn1 = MeanOnlySyncBatchNormalization()
        self.conv2 = Conv2DReparam(channels, [3, 3], padding='SAME')
        # self.bn2 = MeanOnlySyncBatchNormalization()
        self.relu = ReLU()
        if self.flag:
            # self.bn3 = MeanOnlySyncBatchNormalization()
            self.conv3 = Conv2DReparam(channels, [1, 1], stride)
        self.bn4 = MeanOnlySyncBatchNormalization()
...
class ResNet34Rparam(Model):
    def __init__(self):
        super(ResNet34Rparam, self).__init__(name='ResNet34Reparam')
        self.random_flip = tf.keras.layers.RandomFlip(mode="horizontal")

        self.conv1 = Conv2DReparam(64, [7, 7], 2, padding='SAME')
        self.bn = MeanOnlySyncBatchNormalization()
        self.relu = ReLU()
        self.mp1 = MaxPooling2D(3, 2)

        self.conv2_1 = ResBlockRparam(64)
        self.conv2_2 = ResBlockRparam(64)
        self.conv2_3 = ResBlockRparam(64)

        self.conv3_1 = ResBlockRparam(128, 2)
        self.conv3_2 = ResBlockRparam(128)
        self.conv3_3 = ResBlockRparam(128)
        self.conv3_4 = ResBlockRparam(128)

        self.conv4_1 = ResBlockRparam(256, 2)
        self.conv4_2 = ResBlockRparam(256)
        self.conv4_3 = ResBlockRparam(256)
        self.conv4_4 = ResBlockRparam(256)
        self.conv4_5 = ResBlockRparam(256)
        self.conv4_6 = ResBlockRparam(256)

        self.conv5_1 = ResBlockRparam(512, 2)
        self.conv5_2 = ResBlockRparam(512)
        self.conv5_3 = ResBlockRparam(512)
...
def build_resnet34_gmp(optimizer, loss, metrics):
    model = ResNet34Rparam()
    model.compile(
        optimizer=optimizer,
        loss=loss,
        metrics=metrics,
