# This is a modification of the original TensorFlow 2 Implementation of Batch Normalization
...
class MeanOnlyBatchNormalizationBase(Layer):
  ...
  *after having been trained on data that has similar statistics as the
  ...
  def __init__(self,
               ...
               name=None,
               **kwargs):
    super(MeanOnlyBatchNormalizationBase, self).__init__(name=name, **kwargs)
  ...
  def call(self, inputs, training=None):
    ...
    variance = tf.cast(1.0, inputs.dtype) # tf.cast(variance, inputs.dtype)
    if offset is not None:
      offset = tf.cast(offset, inputs.dtype)
    if scale is not None:
      scale = tf.cast(scale, inputs.dtype)
    outputs = tf.nn.batch_normalization(inputs, _broadcast(mean),
                                        variance, offset, scale, # _broadcast(variance), offset, scale,
                                        self.epsilon)
    if inputs_dtype in (tf.float16, tf.bfloat16):
  ...
  def get_config(self):
    ...
    base_config = super(MeanOnlyBatchNormalizationBase, self).get_config()
    return dict(list(base_config.items()) + list(config.items()))


class MeanOnlyBatchNormalization(MeanOnlyBatchNormalizationBase):
  ...
  **About setting `layer.trainable = False` on a `BatchNormalization` layer:**
  ...
  def __init__(self,
               ...
               gamma_constraint=None,
               **kwargs):
    super(MeanOnlyBatchNormalization, self).__init__(
        axis=axis,
        momentum=momentum,
        epsilon=epsilon,
        center=center,
...
class MeanOnlySyncBatchNormalization(MeanOnlyBatchNormalizationBase):
  ...
  def __init__(self,
    ...
    super(MeanOnlySyncBatchNormalization, self).__init__(
        axis=axis,
        momentum=momentum,
        epsilon=epsilon,
        center=center,
